...
float3 SampleBakedGI(float3 positionRWS, float3 normalWS, float2 uvStaticLightmap, float2 uvDynamicLightmap)
{
    ...
    else
    {
        ...
        if (unity_ProbeVolumeParams.w == 1.0)
            return SampleProbeVolumeSH9(unity_ProbeVolumeSH, samplerunity_ProbeVolumeSH, positionRWS, normalWS, GetProbeVolumeWorldToObject(),
                unity_ProbeVolumeParams.y, unity_ProbeVolumeParams.z, unity_ProbeVolumeMin.xyz, unity_ProbeVolumeSizeInv.xyz);
        else
#endif
            return SampleProbeVolumeSH4(unity_ProbeVolumeSH, samplerunity_ProbeVolumeSH, positionRWS, normalWS, GetProbeVolumeWorldToObject(),
                unity_ProbeVolumeParams.y, unity_ProbeVolumeParams.z, unity_ProbeVolumeMin.xyz, unity_ProbeVolumeSizeInv.xyz);
    }
    ...
    bool useRGBMLightmap = true;
        ...
        bakeDiffuseLighting += SampleDirectionalLightmap(unity_Lightmap, samplerunity_Lightmap,
                                                         unity_LightmapInd, samplerunity_Lightmap,
                                                         uvStaticLightmap, unity_LightmapST, normalWS, useRGBMLightmap, decodeInstructions);
        #else
        bakeDiffuseLighting += SampleSingleLightmap(unity_Lightmap, samplerunity_Lightmap, uvStaticLightmap, unity_LightmapST, useRGBMLightmap, decodeInstructions);
        #endif
    #endif

    #ifdef DYNAMICLIGHTMAP_ON
        #ifdef DIRLIGHTMAP_COMBINED
        bakeDiffuseLighting += SampleDirectionalLightmap(unity_DynamicLightmap, samplerunity_DynamicLightmap,
                                                         unity_DynamicDirectionality, samplerunity_DynamicLightmap,
                                                         uvDynamicLightmap, unity_DynamicLightmapST, normalWS, false, decodeInstructions);
        #else
        bakeDiffuseLighting += SampleSingleLightmap(unity_DynamicLightmap, samplerunity_DynamicLightmap, uvDynamicLightmap, unity_DynamicLightmapST, false, decodeInstructions);
        #endif
    #endif
...
float4 SampleShadowMask(float3 positionRWS, float2 uvStaticLightmap) // normalWS not use for now
{
    ...
    {
        rawOcclusionMask = SampleProbeOcclusion(unity_ProbeVolumeSH, samplerunity_ProbeVolumeSH, positionRWS, GetProbeVolumeWorldToObject(),
                                                unity_ProbeVolumeParams.y, unity_ProbeVolumeParams.z, unity_ProbeVolumeMin.xyz, unity_ProbeVolumeSizeInv.xyz);
    }
    else
    {
